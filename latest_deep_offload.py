...
def download_gtsrb(root: str):
    ...
    dest = os.path.join(root, 'GTSRB.zip')
    print('[GTSRB] Downloading …')
    urllib.request.urlretrieve(url, dest)
    print('[GTSRB] Extracting …')
...
def main(epochs_DDNN=  50,epochs_optimization=20, batch_size=256 , L0=0.54 , local_weight=0.7,mode= 'train', dataset_name='gtsrb32'):
    ...
    num_layers=1
    input_mode='img'  # 'logits'|'feat'|'logits_plus'
    ...
    offload_dataset = OffloadDatasetCNN(combined_data, b_star,input_mode= input_mode,
                                    ...
                                    cloud_clf=cloud_cnn,
                                    device=device)
    offload_loader= DataLoader(offload_dataset,batch_size,shuffle= False)
    
    # # ########################## ORACLE METRICS TEST #############################
        
    # metrics_dataset = OffloadDatasetCNN(combined_data, b_star, input_mode=input_mode,include_bk=True)
    
    # metrics = oracle_noise_metrics_old(metrics_dataset,
    #                             local_feature_extractor,
    #                             local_classifier,
    #                             cloud_cnn,
    #                             bk_star=b_star,
    ...
    print(f"Training Deep Offload Mechanism with input_mode='{input_mode}'")
    ...
    #     deep_offload_model = OffloadMechanism(
    #         input_shape=(input_dim,),          # keyword
    #         input_mode=input_mode,        # keyword
    #         fc_dims=[256,128,64,16, 1],        # keyword    # ΝΑ ΔΟΚΙΜΑΣΩ [512,256,128,64,32,1]     ΑΡΧΙΚΟ[128,64,32,1]  
    ...
    #     input_dim= NUM_CLASSES + 2  # logits + margin + entropy
    #     deep_offload_model = OffloadMechanism(
    #         input_shape=(input_dim,),          # keyword
    #         input_mode=input_mode,        # keyword
    ...
    deep_offload_model = OffloadMechanism(input_mode=input_mode, NUM_CLASSES=NUM_CLASSES).to(device)
...
# find the threshold for the deep offload model decision in order to have the same cloud percentage as the oracle
# use the the training  cloud percentage and calculate threshold accordingly
    tau, cloud_ratio = calibrate_threshold(
        deep_offload_model,
        offload_loader,              # loader με τα TRAIN labels
        device='cuda')
    print(f"Training cloud%={100*cloud_ratio:.2f}  →  threshold τ={tau:.4f}")
    local_percentage, ddnn_acc = test_DDNN_with_optimized_rule(
            ...
            cloud_cnn,
            test_loader,
            b_star,threshold=tau,input_mode=input_mode)

    # print(f"DDNN overall accuracy with trained DOM: {ddnn_acc:.2f}% "
    #     f"(local%={local_percentage:.1f})")
    ent_local_acc, ent_cloud_acc, ent_overall_acc, ent_local_classified, ent_cloud_classified,ent_local_percentage,entropy_threshold = test_DDNN_with_entropy(
    local_feature_extractor,
    local_classifier,
    cloud_cnn,
    test_loader,
    ...
    # ################################## Visualize samples with bks and decisions #########################################
    # bk_vals, decisions, img_ids,all_labels = gather_test_decisions(
    #     test_loader,
    #     local_feature_extractor,
    #     local_classifier,
    #     cloud_cnn,
    ...
    #     device=device,
    #     input_mode=input_mode,
    #     match_oracle_to=None,
    #     threshold=tau
    # )

    # fig, mismatch_idx, fig2 = plot_decision_comparison(
    #     bk_vals,
    #     decisions,
    #     b_star=b_star,
    #     show_all=True,            # μόνο mismatches
    #     max_samples_display=50,  # κράτα 100 τυχαία mismatches
    #     sample_ids=img_ids,
    ...
    # os.makedirs("plots", exist_ok=True)         # create plots directory if it does not exist
    # fig.savefig("plots/decision_comparison_all.svg", bbox_inches="tight")
    # fig.savefig("plots/decision_comparison_all.png",  dpi=150, bbox_inches="tight")   # raster
    # if fig2 is not None:
    #     fig2.savefig("plots/comparison_of_classes_results.svg", bbox_inches="tight")
    #     fig2.savefig("plots/comparison_of_classes_results.png", dpi=150, bbox_inches="tight")

    # fig, mismatch_idx, fig2 = plot_decision_comparison(
    #     bk_vals,
    #     decisions,
    #     b_star=b_star,
    #     show_all=False,            # μόνο mismatches
    #     max_samples_display=50,  # κράτα 100 τυχαία mismatches
    #     sample_ids=img_ids,
    #     true_labels=all_labels,
    #     class_names=CIFAR10_LABELS
    ...
    # # save the plot in the plots directory
    # os.makedirs("plots", exist_ok=True)         # create plots directory if it does not exist
    # fig.savefig("plots/decision_comparison_only_mismatch.svg", bbox_inches="tight")
    # fig.savefig("plots/decision_comparison_only_mismatch.png",  dpi=150, bbox_inches="tight")   # raster

################################## Visualize samples with bks and decisions #########################################

############################## COREALTION AND CONFUSION MATRIX #########################################################

    # deep_probs = []
    # deep_offload_model.eval()
    # with torch.no_grad():
    #     for x_batch, *_ in offload_loader:  # 
    #         logits = deep_offload_model(x_batch.to(device))
    #         deep_probs.append(torch.sigmoid(logits).squeeze(1).cpu())
    # deep_probs = torch.cat(deep_probs).numpy()

    # deep_dec     = decisions['Deep-offload']          # π.χ. array shape (N,)
    # oracle_dec   = decisions['Oracle']                # χωρίς balancing!

    # # corr(bk, P(cloud))
    # rho = np.corrcoef(all_bks, deep_probs)[0, 1]
    # print(f"corr(bk, p_cloud) = {rho:+.3f}")   # περιμένουμε θετικό

    # cm = confusion_matrix(oracle_dec, deep_dec, labels=[0,1])
    # print(cm)            # [[TN FP]
    #                     #  [FN TP]]

    # disp = ConfusionMatrixDisplay(cm, display_labels=['local','cloud'])
    # disp.plot(values_format='d', colorbar=False)
    # plt.title("Oracle vs Deep-offload");  plt.show()
